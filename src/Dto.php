...
//        }
//        else {
//            //$this->offsetSet('.', $input);
//            //print __LINE__.' What to do if it is not a hash?'; exit;
//        }
...
//        }
    ...
    {
        $this->log(sprintf("%s(%s, %s, %s)", __FUNCTION__, $index, '-val-', ($force) ? 'true' : 'false'));
    ...
    {
        if (empty($this->template) || isset($this->template[$index])) {
            // This bit allows us to dynamically deepen the object structure
            if (!isset($this[$index])) {
                //if (isset($this->template[$index]) && !isset($this[$index])) {
...
//            $this->log(sprintf("New DTO: %s(%s)", __FUNCTION__, implode(", ", func_get_args())));
//            $classname = get_called_class();
//            return $this->offsetSet($index, new $classname($value, $this->getTemplateSubset($index, $this->template), $this->getMetaSubset($index,$this->meta)));
//
//        }
//        else {
//            // PHP gotcha: isset comes back false when the value is set to null
...
//        }
    ...
    protected function setTypeHash($value, $index)
    {
        ...
        $classname = get_called_class();
        // TODO: the $template and $meta should be subsets!  Not the originals!
        //return new $classname($value->toArray(), $template, $meta);
        //print $index ."\n"; print_r($value); print_r($this->meta); exit;
        if ($value instanceof \Dto\Dto) {
